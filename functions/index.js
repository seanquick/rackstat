...
// optional: keep your cost control
...
exports.deleteUserData = onCall(async (request) => {
    ...
    if (role === "player" || role === "athlete") {
        ...
        const w1 = await db.collection("completed_workouts")
            .where("uid", "==", targetUid).get();

        w1.forEach(doc => batch.delete(doc.ref));

        const w2 = await db.collection("completed_workouts")
            .where("athleteId", "==", targetUid).get();

        w2.forEach(doc => batch.delete(doc.ref));
